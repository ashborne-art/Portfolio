...
                
                // ✅ Add a check for a successful response (status code 200-299)
                if (!response.ok) {
                    console.error('Server responded with an error:', response.status, response.statusText);
                    setCertifications([]); // Set to empty array to prevent TypeError
                    return;
                }

                const data = await response.json();
                
                // ✅ Add a check to ensure the data is an array
                if (Array.isArray(data)) {
                    setCertifications(data);
                } else {
                    console.error('Fetched data is not an array:', data);
                    setCertifications([]); // Set to empty array for safe rendering
                }
            } catch (error) {
                console.error('Error fetching certifications:', error);
                setCertifications([]); // Always set to empty array on error
            }
        };
        fetchCertifications();
    }, []);
